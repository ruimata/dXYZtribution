...
class OBJECT_PT_distribution_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout = self.layout
        layout.label(text="Distribution Modes")
        ...
        if bpy.context.selected_objects.__len__() < 2:
            ...
            sub_layout.prop(
                context.scene,
                "positional",
                text="In Place  (within max/min positions)",
            )
        else:
            gap_choice = True
            layout.prop(
        ...
        # Spacing float can only be available when positional is false (even gaps) or when only two
        ...
        if not context.scene.positional or not gap_choice:
            layout.prop(context.scene, "spacing", text="Spacing", expand=False)
...
class OBJECT_OT_distribution(bpy.types.Operator):
    ...
    bpy.types.Scene.positional = bpy.props.BoolProperty(
        description="SELECTED: Use original object's positions, in each axis, and use coordinates of objects at max and min positions as references and distribute all other objects between them.\n\nUNSELECTED: User must specify an intended separation between objects.\n\n",
        default=True,
    )
    ...
    bpy.types.Scene.spacing = bpy.props.FloatProperty(
        name="Spacing", default=1, min=0, step=1, precision=3
    )
...
def apply_translations(sorted_objs, axis, positional, apply_to, spacing):
    if positional:
...
def unregister():
    bpy.utils.unregister_class(OBJECT_OT_distribution)
    bpy.utils.unregister_class(OBJECT_PT_distribution_panel)
    bpy.utils.unregister_class(ApplyDistribution)
